...
func TestClaudeTerminalStreamErrorDoesNotRewriteCommittedStatus(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	...
	trackingWriter := &statusTrackingWriter{ResponseWriter: c.Writer}
	...
	errs <- &interfaces.ErrorMessage{
		StatusCode: http.StatusBadRequest,
		Error:      errors.New(`{"error":{"message":"Your input exceeds the context window of this model.","type":"invalid_request_error"}}`),
	}
	close(errs)

	handler.forwardClaudeStream(c, trackingWriter, func(error) {}, data, errs)

	if len(trackingWriter.statuses) != 0 {
		t.Fatalf("attempted to rewrite committed HTTP status: %v", trackingWriter.statuses)
	}
	if recorder.Code != http.StatusOK {
		t.Fatalf("status = %d, want %d", recorder.Code, http.StatusOK)
	}
	if !bytes.Contains(recorder.Body.Bytes(), []byte("event: error\n")) {
		t.Fatalf("terminal SSE error missing; body=%s", recorder.Body.String())
	}
}

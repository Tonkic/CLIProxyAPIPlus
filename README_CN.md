...
本仓库是 **CLIProxyAPI Plus**，是 `router-for-me/CLIProxyAPI` 的 fork，由 `https://github.com/Tonkic/CLIProxyAPIPlus` 维护。Plus 版本保持上游行为，同时增加 GitHub Copilot、Kiro 等平台支持。
...
独立的 CLIProxyAPI 使用量持久化与可视化服务，定期同步 CLIProxyAPI 数据，存储到 SQLite，提供聚合 API，并内置使用量分析与统计仪表盘。CLIProxyAPI Plus 使用 Redis 兼容 usage queue 作为对接点，并依赖 CPA Usage Keeper 提供持久化 usage 存储和可视化。

...
func TestApplyClaudeHeaders_EnablesOneMillionContextFromIncomingHeader(t *testing.T) {
	req := newClaudeHeaderTestRequest(t, nil)
	auth := &cliproxyauth.Auth{}
	incoming := make(http.Header)
	incoming.Set("X-CPA-CLAUDE-1M", "true")

	if err := applyClaudeHeaders(req, auth, "test-key", false, nil, nil, &config.Config{}, incoming, false); err != nil {

...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if len(oldCfg.GeminiKey) != len(newCfg.GeminiKey) {
		...
		for i := range oldCfg.GeminiKey {
			...
			changes = appendOptionalBoolChange(changes, fmt.Sprintf("gemini[%d].session-affinity", i), o.SessionAffinity, n.SessionAffinity)
	...
	if len(oldCfg.InteractionsKey) != len(newCfg.InteractionsKey) {
		...
		for i := range oldCfg.InteractionsKey {
			...
			changes = appendOptionalBoolChange(changes, fmt.Sprintf("interactions[%d].session-affinity", i), o.SessionAffinity, n.SessionAffinity)
			if strings.TrimSpace(o.APIKey) != strings.TrimSpace(n.APIKey) {
	...
	if len(oldCfg.ClaudeKey) != len(newCfg.ClaudeKey) {
		...
		for i := range oldCfg.ClaudeKey {
			...
			changes = appendOptionalBoolChange(changes, fmt.Sprintf("claude[%d].session-affinity", i), o.SessionAffinity, n.SessionAffinity)
			if strings.TrimSpace(o.APIKey) != strings.TrimSpace(n.APIKey) {
	...
	if len(oldCfg.CodexKey) != len(newCfg.CodexKey) {
		...
		for i := range oldCfg.CodexKey {
			...
			changes = appendOptionalBoolChange(changes, fmt.Sprintf("codex[%d].session-affinity", i), o.SessionAffinity, n.SessionAffinity)
			if strings.TrimSpace(o.APIKey) != strings.TrimSpace(n.APIKey) {
	...
	if len(oldCfg.XAIKey) != len(newCfg.XAIKey) {
		...
		for i := range oldCfg.XAIKey {
			...
			changes = appendOptionalBoolChange(changes, fmt.Sprintf("xai[%d].session-affinity", i), o.SessionAffinity, n.SessionAffinity)
	...
	if len(oldCfg.VertexCompatAPIKey) != len(newCfg.VertexCompatAPIKey) {
		...
		for i := range oldCfg.VertexCompatAPIKey {
			...
			changes = appendOptionalBoolChange(changes, fmt.Sprintf("vertex[%d].session-affinity", i), o.SessionAffinity, n.SessionAffinity)
			if strings.TrimSpace(o.APIKey) != strings.TrimSpace(n.APIKey) {

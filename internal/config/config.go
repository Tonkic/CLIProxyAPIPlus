...
type Config struct {
	...
	// KiroKey defines Kiro (AWS CodeWhisperer/Amazon Q) credentials.
	KiroKey []KiroKey `yaml:"kiro" json:"kiro"`

	// KiroFingerprint pins the client fingerprint used by Kiro requests.
	...
	// AmpCode contains Amp CLI upstream routing and model mappings.
	...
	// IncognitoBrowser opens OAuth URLs in a private browser session when supported.

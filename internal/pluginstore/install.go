...
func installTargetPath(options InstallOptions, id string) (string, error) {
	...
	currentPath, errCurrent := currentPluginFilePath(options.PluginsDir, id, options.GOOS)
	if errCurrent != nil {
		return "", errCurrent
	}
...
func currentPluginFilePath(root string, id string, goos string) (string, error) {
	...
	for _, file := range files {
		...
		if filepath.Base(file.Path) == exactName {

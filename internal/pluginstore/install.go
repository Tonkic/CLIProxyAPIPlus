...
func installTargetPath(options InstallOptions, id string, version string) (string, error) {
	defaultPath := filepath.Join(options.PluginsDir, options.GOOS, options.GOARCH, versionedPluginFileName(id, version, options.GOOS))
	if options.GOOS != runtime.GOOS || options.GOARCH != runtime.GOARCH {
		return defaultPath, nil
	}
	currentPath, errCurrent := currentPluginFilePath(options.PluginsDir, id, version, options.GOOS)
	if errCurrent != nil {
		return "", errCurrent
	}
...
func currentPluginFilePath(root string, id string, version string, goos string) (string, error) {
	id = strings.TrimSpace(id)
	extension := pluginExtension(goos)
	exactName := id + extension
	versionedName := versionedPluginFileName(id, version, goos)
	files, errDiscover := discoverCurrentPluginFiles(root)
	if errDiscover != nil {

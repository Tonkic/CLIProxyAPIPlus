...
func (h *Host) callManagementHandler(ctx context.Context, record managementRouteRecord, req pluginapi.ManagementRequest) (resp pluginapi.ManagementResponse, err error) {
	if h == nil || record.route.Handler == nil || h.isPluginFused(record.pluginID) || !h.routeRecordCurrent(record.pluginID, record.path, record.version) {
...
func (h *Host) routeRecordCurrent(pluginID string, path string, version string) bool {
	if strings.TrimSpace(path) == "" && strings.TrimSpace(version) == "" {
		return strings.TrimSpace(pluginID) != ""
	}
	return h.pluginIdentityCurrent(pluginID, path, version)
}

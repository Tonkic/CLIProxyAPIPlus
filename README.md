...
This repository is **CLIProxyAPI Plus**, a fork of `router-for-me/CLIProxyAPI` maintained at `https://github.com/Tonkic/CLIProxyAPIPlus`. It keeps upstream behavior while adding Plus-specific platform support such as GitHub Copilot and Kiro.
...
Standalone persistence and visualization service for CLIProxyAPI, with periodic data sync, SQLite storage, aggregate APIs, and a built-in dashboard for usage and statistics. CLIProxyAPI Plus uses the Redis-compatible usage queue as the integration point and relies on CPA Usage Keeper for persistent usage storage and visualization.
